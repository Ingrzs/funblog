...
import type { MemeCreationData } from '../types';
 ...
 */
// Fix: Add types to loadImage function signature
 ...
 */
// Fix: Add types to calculateWrappedText function signature
const calculateWrappedText = (context: CanvasRenderingContext2D, text: string, maxWidth: number): string[] => {
 ...
 */
// Fix: Add types to drawWrappedText function signature
const drawWrappedText = (ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number, textAlign: 'left' | 'center' | 'right' | 'justify'): number => {
 ...
 * @param {MemeCreationData} memeData Un objeto que contiene todos los datos necesarios para crear el meme.
 * @returns {Promise<string>} Una promesa que se resuelve con la URL de datos de la imagen generada.
 */
// Fix: Add types to createMemeImage function signature
export const createMemeImage = async (memeData: MemeCreationData): Promise<string> => {
    ...
    try {
        ...
        ctx.font = textFont;
        // Fix: Correctly calculate text height without drawing. The previous method accessed a '.height' property on a number.
        const textHeight = calculateWrappedText(ctx, text, contentWidth).length * textLineHeight; // Pre-calculate height
 ...
 * @param {MemeCreationData} phraseData Un objeto que contiene los datos para crear la imagen de la frase.
 * @returns {Promise<string>} Una promesa que se resuelve con la URL de datos de la imagen generada.
 */
export const createPhraseImage = async (phraseData: MemeCreationData): Promise<string> => {
 ...
 */
// Fix: Add types to drawRoundRect function signature
 ...
 * @param {MemeCreationData} tweetData Objeto con los datos para crear el tweet.
 * @returns {Promise<string>} Una promesa que se resuelve con la URL de datos de la imagen generada.
 */
// Fix: Add types to createTweetImage function signature
export const createTweetImage = async (tweetData: MemeCreationData): Promise<string> => {
